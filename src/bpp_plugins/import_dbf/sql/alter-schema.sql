...
SELECT * from bpp_plugins.import_dbf_b_a WHERE idt_jed LIKE 'X%';
DELETE from bpp_plugins.import_dbf_b_a WHERE idt_jed LIKE 'X%';
...
SELECT * from bpp_plugins.import_dbf_b_u WHERE idt LIKE 'X%';
DELETE from bpp_plugins.import_dbf_b_u WHERE idt LIKE 'X%';
...
update import_dbf_aut set idt_jed = (select idt_jed from bpp_plugins.import_dbf_jed where skrot = '000') where idt_jed is NULL;
update import_dbf_b_a set idt_jed = (select idt_jed from bpp_plugins.import_dbf_jed where skrot = '000') where idt_jed is NULL;
...
UPDATE import_dbf_aut SET ref = NULL WHERE ref not in (select idt_aut from bpp_plugins.import_dbf_aut);

-- Ustawia jednostke 'brak jednostki' tam gdzie jest NULL jako jednostka w tabeli b_a
UPDATE import_dbf_b_a SET idt_jed = (SELECT idt_jed from bpp_plugins.import_dbf_jed WHERE UPPER(nazwa) = 'BRAK JEDNOSTKI') WHERE idt_jed IS NULL;
